...
export function PredictionResult({ result }: { result: PredictResult }) {
  ...
  return (
    <div className="space-y-6">
                ...
                data={[
                  {
                    name: "score",
                    value: pct,
                    fill: approved ? "var(--color-success)" : "var(--color-destructive)",
                  },
                ]}
                startAngle={220}
                endAngle={-40}
        ...
        >
          ...
          <ul className="mt-4 space-y-3">
            {(result.suggestions.length
              ? result.suggestions
              : ["Your profile is well-positioned across all key signals."]
            ).map((s, k) => (
      ...
      {/* Feature contributions chart — only when backend provides SHAP values */}
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-muted-foreground">
              Positive ↑ approval · Negative ↓ approval
            </span>
          </div>
                ...
                <Bar dataKey="value" radius={[6, 6, 6, 6]}>
                  {result.shap_values
                    ...
                    .map((s, i) => (
                      <Cell
                        key={i}
                        fill={s.value >= 0 ? "var(--color-success)" : "var(--color-destructive)"}
                      />
                    ))}
                </Bar>
              </BarChart>
